...
- **Enter**: 選択したファイル/フォルダへジャンプ（`*jumppath` によりファイルの場合は親フォルダへ移動してカーソルを合わせる）
- **ESC**: 中止
...
| ODT・RTF・JTD 等 | xdoc2txt でテキスト変換して `PPv[Z]` に表示 |
| DOC・DOCX・XLS・XLSX・PPT・PPTX（Office 系）・PDF 等 | `*appview`（IPreviewHandler による表示） |
