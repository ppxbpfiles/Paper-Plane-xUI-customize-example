...
- **Git をお使いの方:**
  ```bash
  git clone https://github.com/ppxbpfiles/Paper-Plane-xUI-customize-example.git
  ```
- **ZIP でダウンロードする場合:**
...
`%Obsqn`（バックグラウンド実行・終了待ち）にマークファイルを展開する `%FC` を組み合わせることで、マークしたファイルを1件処理するたびに次のファイルへ進みます。処理が終わったエントリは `*trimmark` でマークを外すため、途中でエラーが発生した場合でもどこまで処理が済んだか一目で確認できます。
...
%Obsqn ツール %FC オプション "%so"param"" %:*trimmark
```
