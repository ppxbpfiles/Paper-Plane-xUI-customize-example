...
| ID | 用途 | 表示モード |
|---|---|---|
| `PPv[A]`〜`[I]` | `[V]` キーで起動。未使用 ID を自動割り当て、複数同時起動可 | キャレット |
| `PPv[J]`〜`[X]` | MIEL モード連動ビュー。`[Shift+Y]` で X から逆順に未使用 ID を割り当て（J より前には割り当てない） | ページ |
| `PPv[Y]` | `[Y]` キーで起動。固定 ID、ホイールで前後ファイル切り替え | ページ |
| `PPv[Z]` | チッププレビュー（`[Shift+V]`）専用 | ページ |
