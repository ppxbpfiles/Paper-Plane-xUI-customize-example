...
必須ではないがあれば便利なもの。CFG から `%0script\...` の形で参照されるため、`script` サブフォルダに配置してください。
...
コピー先・移動先にはディレクトリではなくファイルのフルパスをそのまま指定できます。その場合、指定したファイルが存在するディレクトリがコピー先・移動先として使われます。上記の Everything 検索結果からファイルを見つけたとき、そのファイルと同じフォルダに別のファイルを送りたい場合に、わざわざディレクトリを辿らずにファイルパスをそのまま使えます。
...
独立窓（`PPc[A]` など）で `[C]` を押すと、対応するタブ窓（`PPc -combo`）の現在地がサブ窓の起点として自動的に使われます。タブ窓に仕分け先のフォルダをあらかじめ複数タブで開いておけば、独立窓側で仕分け元を操作しながら `[Ctrl+←]` / `[Ctrl+→]` でタブを切り替えて任意のフォルダを素早くコピー先に指定できます。
...
`fd` を使ったサーチ機能が以下の場面で利用できます。

**① `[Ctrl+J]` — サブディレクトリのインタラクティブサーチ**

`[Ctrl+J]` キーを押すと、`fd` でカレントフォルダ以下の全てのファイル・フォルダを列挙し、PPx 内蔵の一行編集補完でインクリメンタル検索できます。外部 TUI ツールを使わず PPx 内で完結します。

`fd` の出力はそのまま補完リストにするのではなく、`filepathlist_to_dircomplist.js` でファイル名部分を行頭コメントに分離してから使用します。これにより、補完候補にはファイル名が見えつつ、実際に入力（ジャンプ先）に使われるのはそのファイルが属するディレクトリパスのみになります。ファイル名まで一致するかどうかで絞り込みつつ、余計な入力なしにそのファイルがあるフォルダへ直接移動できます。
...
コピー（`[C]`）・移動（`[M]`）で開く一行編集で `[F8]` を押すと、カーソル上のファイル名（英字＋バージョン番号を想定）から拡張子違い・バージョン違いのファイルを Everything で検索し、ヒットしたファイルのあるフォルダを補完候補として表示します。`filepathlist_to_dircomplist.js` でファイル名をコメント化した上で候補に出すため、どのファイルが見つかったか確認しながら宛先フォルダを選べます。旧バージョンの保管フォルダにコピー・移動する場合など、特定用途向けの補助機能です。
...
ここではペイン数（`PPx.Pane.Count`）を例に取ります。ペイン数は PPx 本体のマクロ文字では取得できませんが、`%*js(...)` インラインスクリプトを使うことでマクロ文字と同様の感覚で利用できます。
...
この例では `[TAB]` キーを押したとき、ペインが 2 つ開いている場合はもう一方のペインへフォーカスを移動し（`*pane focus ~`）、それ以外の場合は通常の `[TAB]` 動作（`%K"@TAB"`）にフォールバックします。
