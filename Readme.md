...
- 目的別にタブグループ（A〜Z）を切り替えて管理できるタブ窓（→ [タブグループの切り替え](#タブグループの切り替え)）
...
| `PPc[Z??]`（タブ窓） | タブ付きウィンドウ。A〜Z のタブグループで目的別に管理 |
...
タブ付きウィンドウ（`PPc -combo`）では A〜Z のタブグループを切り替えて目的別にフォルダ構成を管理できます（→ [タブグループの切り替え](#タブグループの切り替え)）。
...
各グループには任意の名前を付けられます。ウィンドウメニューの「タブグループ名の編集」から ID（A〜Z）を指定して入力してください。設定した名前はメニューに反映されます（例：`tabgroup A:作業中`）。
...
```
%I"%*extract("%*regexp("%*ppxlist(-C)","s/[^,]*C_([^,]+),/%%*name(DC,%%DC$1);/g")")"
```
